...
class PrimaryNet(nn.Module):
    def __init__(self,opt):
        ...
        self.conv0 = nn.Sequential(
                repPad, nn.Conv3d(1,8,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(8),nn.LeakyReLU(),
                repPad, nn.Conv3d(8,8,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(8),nn.LeakyReLU())
        ...
        self.convL0 = nn.Sequential(
                repPad, nn.Conv3d(8,16,(1,3,3),stride=(1,2,2),padding=0),nn.InstanceNorm3d(16),nn.LeakyReLU(), #192,48
                repPad, nn.Conv3d(16,16,(1,3,3),stride=(1,1,1),padding=0),nn.InstanceNorm3d(16),nn.LeakyReLU(),
                repPad, nn.Conv3d(16,32,(1,3,3),stride=(1,2,2),padding=0),nn.InstanceNorm3d(32),nn.LeakyReLU(),#96,24
                repPad, nn.Conv3d(32,32,(1,3,3),stride=(1,1,1),padding=0),nn.InstanceNorm3d(32),nn.LeakyReLU(),
                repPad, nn.Conv3d(32,48,(1,3,3),stride=(1,2,2),padding=0),nn.InstanceNorm3d(48),nn.LeakyReLU())#48,12
                
        self.convL0Up = nn.Sequential(
                nn.ConvTranspose3d(48, 32,(1,7,7), stride=(1,4,4), padding=(0,3,3), output_padding=(0,3,3)),
                nn.ConvTranspose3d(32, 16,(1,5,5), stride=(1,2,2), padding=(0,2,2), output_padding=(0,1,1)))
        
        
        
        self.convH1 = nn.Sequential(
                repPad, nn.Conv3d(24,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU(),
                repPad, nn.Conv3d(12,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU(),
                repPad, nn.Conv3d(12,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU(),
                repPad, nn.Conv3d(12,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU())
        
        self.convL1 = nn.Sequential(
                repPad, nn.Conv3d(48,48,(1,3,3),stride=(1,1,1),padding=0),nn.InstanceNorm3d(48),nn.LeakyReLU(),
                repPad, nn.Conv3d(48,48,(1,3,3),stride=(1,1,1),padding=0),nn.InstanceNorm3d(48),nn.LeakyReLU(),
                repPad, nn.Conv3d(48,48,(1,3,3),stride=(1,1,1),padding=0),nn.InstanceNorm3d(48),nn.LeakyReLU(),
                repPad, nn.Conv3d(48,48,(1,3,3),stride=(1,1,1),padding=0),nn.InstanceNorm3d(48),nn.LeakyReLU())
        ...
        self.convH2 = nn.Sequential(
                repPad, nn.Conv3d(28,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU(),
                repPad, nn.Conv3d(12,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU(),
                repPad, nn.Conv3d(12,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU(),
                repPad, nn.Conv3d(12,12,(1,3,3),stride=1,padding=0),nn.InstanceNorm3d(12),nn.LeakyReLU())
        ...
        self.convProj=nn.Sequential(repPad, nn.Conv3d(28,1,(1,3,3),stride=1,padding=0))
    ...
    def forward(self,xIn,target=None):
        ...
        xL0=self.convL0(x0)
        #lateral connection
        xH0=torch.cat([xH0,self.convL0Up(xL0)],dim=1)
        ...
        xL1=self.convL1(xL0)
        #lateral connection
        xH1=torch.cat([xH1,self.convL1Up(xL1)],dim=1)
        ...
        xL2=self.convL1(xL1)
        #lateral connection
        x=torch.cat([xH2,self.convL2Up(xL2)],dim=1)
...
class STNNet(nn.Module):
    def __init__(self,opt):
        ...
        self.conv0 = nn.Sequential(
                 nn.Conv2d(56,96,(3,3),stride=2,padding=(1,1)),nn.LeakyReLU(),
                 nn.Conv2d(96,32,(3,3),stride=1,padding=(1,1)),nn.LeakyReLU(),
                 nn.Conv2d(32,32,(3,3),stride=1,padding=(1,1)),nn.LeakyReLU(),
                 nn.Conv2d(32,32,(3,3),stride=1,padding=(1,1)),nn.LeakyReLU())
        self.conv1 = nn.Conv2d(32, 2, 3, 1, 1)
...
class FusionNet(nn.Module):
    ...
    def __init__(self,opt):
        ...
        self._opt=opt
        repPad=nn.ReplicationPad3d((1,1,1,1,0,0))
        self.conv0=nn.Sequential(repPad, nn.Conv3d(28,28,(3,3,3),stride=1,padding=0),
                                 nn.InstanceNorm3d(28),nn.LeakyReLU())
        self.conv1=nn.Sequential(repPad, nn.Conv3d(28,28,(3,3,3),stride=1,padding=0),
                                 nn.InstanceNorm3d(28),nn.LeakyReLU())
        self.conv2=nn.Sequential(repPad, nn.Conv3d(28,28,(3,3,3),stride=1,padding=0),
                                 nn.InstanceNorm3d(28),nn.LeakyReLU())
        self.conv3=nn.Sequential(repPad, nn.Conv3d(28,28,(3,3,3),stride=1,padding=0),
                                 nn.InstanceNorm3d(28),nn.LeakyReLU())
        
        self.conv4=nn.Conv3d(28,1,(1,3,3),stride=1,padding=(0,1,1))
...
class FusionRefineNet(nn.Module):
    ...
    def __init__(self,opt):
        ...
        self._opt=opt
        repPad=nn.ReplicationPad3d((1,1,1,1,0,0))
        self.conv0=nn.Sequential(repPad, nn.Conv3d(29,28,(3,3,3),stride=1,padding=0),
                                 nn.InstanceNorm3d(28),nn.LeakyReLU())
        self.conv1=nn.Sequential(repPad, nn.Conv3d(28,28,(3,3,3),stride=1,padding=0),
                                 nn.InstanceNorm3d(28),nn.LeakyReLU())
        self.conv2=nn.Sequential(repPad, nn.Conv3d(28,28,(3,3,3),stride=1,padding=0),
